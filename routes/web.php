...
Route::middleware(['admin_auth','permission_auth'])->prefix('admin')->group(function(){
    ...
    /*########################[权限相关]########################*/
    //权限列表
    Route::get('/permissions/list','Admin\PermissionController@list')->name('admin.permission.list');

    //获取权限的数据
    Route::any('/get/permission/list/{fid?}','Admin\PermissionController@getPermissionList')->name('admin.get.permission.list');
    
    //权限添加
    Route::get('/permissions/create','Admin\PermissionController@create')->name('admin.permission.create');
    ...
    Route::any('goods/sku/list/bind/{id}','Admin\GoodsSkuController@getSkuList')->name('admin.goods.sku.list.bind');
    ...
    //地区管理
    Route::get('region/list/{pid?}','Admin\RegionController@list')->name('admin.region.list');
    //添加页面
